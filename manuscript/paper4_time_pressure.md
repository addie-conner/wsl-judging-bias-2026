...
_Companion to "Detecting an Unannounced Judging Reform: Panel-Composition Change in the WSL Championship Tour, 2009–2026" (Conner 2026, this archive)._
...
**Question.** Round-number anchoring is the most replicable bias signature in professional surf judging — 31.4% of individual judge scores end in `.0` against a 10% uniform null (whole-anchor excess of 21.4 percentage points; companion paper, this archive). The cognitive-science literature on anchoring proposes that respondents under cognitive load produce round, easily-retrieved candidate values more often than they do under deliberation (Tversky and Kahneman 1974; Englich, Mussweiler, and Strack 2006), and treats round numbers themselves as goal anchors (Pope and Simonsohn 2011). We test whether the surf-judging anchoring rate is in fact driven by judge time-pressure.
...
Round-number anchoring — the empirical regularity that humans asked to produce a number cluster on integer-rounded values far more often than uniform-distribution baselines predict — has been documented across domains as varied as judicial sentencing, real-estate listings, self-reported demographic data, and SAT retake decisions (Pope and Simonsohn 2011; Englich, Mussweiler, and Strack 2006). The mechanism most commonly invoked, following Tversky and Kahneman's (1974) anchoring-and-adjustment framework, is that round numbers are retrieved more cheaply from working memory than non-round numbers and require fewer cognitive steps to generate. Englich, Mussweiler, and Strack (2006) further show that anchoring effects on expert judges are *amplified* under conditions of time pressure or cognitive load — the same judges who partially correct away from irrelevant anchors when deliberating fail to do so when their cognitive resources are taxed. This is the field's strongest experimental basis for the prediction that judges scoring waves under time pressure should anchor more.

This framing is intuitively applicable to professional surf judging. Five judges score each wave on a continuous 0.00–10.00 scale within roughly thirty seconds of the wave breaking. The companion paper to this analysis (Conner 2026, "Detecting an Unannounced Judging Reform") reports that 59.9% of individual judge scores end in `.0`, `.25`, `.5`, or `.75` against a 20% uniform null (3.0× excess; n = 301,478 individual judge decisions, 2009–2026), and that the panel-trim-mean rate of 22.7% on `.0` or `.5` replicates on a temporally held-out 2025 women's Championship Tour sample (25.2%). Whether this clustering is *driven* by time-pressure, however, has not been tested empirically in any subjective-sport judging corpus that we are aware of.
...
A pre-registration covering a confirmatory phase — held-out replication, alternative operationalizations, multiple-comparison correction, and pre-specified sensitivity analyses — was sealed at git commit `c0d1e1a` on 2026-05-07 UTC, before the confirmatory analysis script was written or run. The substantive content of the pre-registration (hypotheses, decision rules, FDR procedure, sensitivity families) was finalized in that commit; the analysis script that operationalized it was a parameterization of the existing discovery script applied to a held-out year-slice. Readers should note that the elapsed time between the pre-registration commit and the first confirmatory result is short (~22 minutes); the pre-registration is substantive in content, but its timing was tight, and we treat it as a credible commitment device rather than as a multi-week deliberation window. Confirmatory findings are reported in §4. A deviation from the sealed pre-registration (the 2026 hold-out estimated at n ≈ 17,000 was not available; the 2025 men's + women's CT was substituted, n = 14,106) is reported in §4.1 and §6.
...
We argue this substitution does not compromise the out-of-sample status of the confirmatory test, for three reasons. First, the substituted sample is *year-disjoint* from the discovery sample (2022–2024 discovery / 2025 hold-out), preserving the pre-registered split structure even though the specific year changed. Second, the substituted sample size (n = 14,106) is within 20% of the originally projected sample size (n ≈ 17,000); a power calculation for the smallest pre-registered effect (H2 fatigue, expected coefficient ≈ +0.003) shows minimum detectable effect at α = 0.05 / 80% power of 0.0029 at n = 14,106 versus 0.0026 at n = 17,000, a difference that does not change the confirm/reject verdict on any of the three primary tests. Third, the primary tests, fixed-effect specifications, FDR correction, and sensitivity-analysis families are all run *exactly* as pre-specified — the deviation is in the sample year, not in the analytical pipeline.

We acknowledge that this is nonetheless a deviation from a literal reading of the sealed pre-registration. A reader who treats the year specification as load-bearing should weight the confirmatory results accordingly. A reader who treats the year-disjoint split as the substantive commitment will find that commitment honored. We report this transparently here and again in §6.
...
The reform implication is direct: the natural operational alternative to an architectural fix — extend the per-wave deliberation window — has been ruled out by the present analysis. Extending the typical wave-to-wave gap from twenty-four seconds to seven minutes (Q1 to Q4 of the wave-density distribution in the 2022–2025 corpus) shifts anchoring by approximately 1.7 pp on a 31% baseline. You cannot run the clock long enough to fix this at the operational layer. The architectural fix — judges scoring in unit increments on a 0–100 input scale with display divided by 10, or an analogous integer-scale design — is the only intervention with a defensible expected effect, since it removes the cluster-on-half-points pathway at the input layer rather than relying on operational mitigation.

### 7.5 What this means for the cognitive-science literature

The Englich, Mussweiler, and Strack (2006) cognitive-load account — that expert judges anchor more when their deliberative resources are taxed — generates a clear field-data prediction: within a single judging environment, stratifying on time-pressure should produce monotone anchoring shifts. We find that prediction at most weakly supported (H2 fatigue, ~1.8 pp), partially refuted (H1 wave-density does not replicate on hold-out), and reversed in the strongest test (H3 end-of-heat). One reading is that surf judging is a context in which anchoring is expressed regardless of time-pressure — driven instead by the cognitive cost of producing a continuous-scale subjective score *at all*, rather than by deliberation time per score. A second reading is that the laboratory-style time-pressure manipulations Englich et al. used are not faithfully reproduced by the field proxies available to us: judges in a heat are not the same as judges in a lab forced to respond within a strict cutoff. Both readings predict the same operational implication: do not expect extending the deliberation window to materially reduce anchoring. The architectural fix — changing the input scale — is the only intervention with a defensible expected effect.
...
Conner, A. (2026). *Detecting an Unannounced Judging Reform: Panel-Composition Change in the WSL Championship Tour, 2009–2026.* Companion paper, this archive.
